    ...
    <div className="animate-soft-in space-y-6">
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <h1 className="mt-3 text-2xl font-bold tracking-tight text-finance-950">{transaction.invoiceNumber}</h1>
          <p className="mt-1 text-sm text-finance-500">{format(new Date(transaction.date), 'dd MMM yyyy, HH:mm')}</p>
        </div>
        <div className="flex flex-col gap-2 sm:flex-row">
          <Button variant="outline" onClick={handleDownloadInvoice} disabled={isDownloadingInvoice} className="gap-2">
            <Download size={16} />
